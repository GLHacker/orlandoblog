...
import { InsertUser, users, posts, InsertPost, attachments, InsertAttachment } from "../drizzle/schema";
...
export async function getAttachmentsByPost(postId: number) {
  ...
  const result = await db.select().from(attachments).where(eq(attachments.postId, postId));
  return result;
}
